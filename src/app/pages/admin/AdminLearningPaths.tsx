...
export function AdminLearningPaths() {
  ...
  const doBatch = async (action: "publish" | "unpublish" | "delete") => {
    ...
    if ("delete" === action && !window.confirm(`确认批量删除 ${selectedIds.length} 条学习路线吗？`)) {
      return;
    }
    setLoading(true);
    setError("");
    try {
      await adminApi.batchLearningPaths(token, { ids: selectedIds, action });
      setSelectedIds([]);
      await reload();
    } catch (e) {
  ...
  const downloadFailedJson = () => {
    ...
    a.href = url;
    a.download = `learning-paths-import-failed-${Date.now()}.json`;
    a.click();
  ...
  const retryFailedRows = async () => {
    ...
    setError("");
    try {
      const resp = (await adminApi.importLearningPaths(token, { items: retryItems, mode: "upsert" })) as any;
  ...
  const importFromText = async () => {
    ...
    try {
      ...
      const resp = (await adminApi.importLearningPaths(token, { items, mode: importMode })) as any;
      setImportText("");
      await reload();
  ...
  return (
    <div className="space-y-4">
      <Card className="rounded-3xl border-border p-5 bg-white">
        ...
        <div className="mt-4">
          <p className="text-xs text-muted-foreground mb-2">批量导入（JSON数组或CSV，至少包含 slug,title）</p>
