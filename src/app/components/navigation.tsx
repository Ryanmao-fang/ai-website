...
export default function Navigation({ currentPage, onNavigate }: NavigationProps) {
  return (
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-[#22345F] bg-[#060B1A]/90 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 md:px-6">
        <button className="flex items-center gap-2" onClick={() => onNavigate("home")}>
          ...
          <span className="text-sm font-semibold text-[#EAF1FF] md:text-base">commononesAI</span>
        </button>
        <div className="flex items-center gap-6 md:gap-8">
          <button
            onClick={() => onNavigate("home")}
            className={`relative text-sm transition-colors duration-300 ${
              "home" === currentPage ? "text-[#6EA0FF]" : "text-[#AFC0E8] hover:text-[#6EA0FF]"
            }`}
          >
            首页
          ...
          <button
            ...
            className={`relative text-sm transition-colors duration-300 ${
              "custom" === currentPage ? "text-[#6EA0FF]" : "text-[#AFC0E8] hover:text-[#6EA0FF]"
            }`}
          >
            定制需求
            {"custom" === currentPage && (
